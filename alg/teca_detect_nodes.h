 ...
 * detect_nodes is used for the detection of nodal features.
 * Candidate points are selected based on information at a single time slice.
 * Typically detect_nodes is followed by stitch_nodes
 * to connect candidate points in time.
 */
...
public:
    ...
    /** @name candidate_threshold_cmd
     * Set the threshold commands
     * [var,op,value,dist;...]
     */
    ///@{
    TECA_ALGORITHM_PROPERTY(std::string, candidate_threshold_cmd)
    ///@}
...
private:
    ...
    std::string candidate_threshold_cmd;
