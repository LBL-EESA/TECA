 ...
 */
class SimplePathSegment : public std::pair<TimeCandidatePair, TimeCandidatePair>
{
public:
    // Constructor.
    SimplePathSegment(
    	...
    	int iCandidate0,
    	int iTime1,
    	int iCandidate1) : std::pair<TimeCandidatePair, TimeCandidatePair>(
...
public:
    ...
    void Parse(const std::string &strOp,
               const std::vector<std::string> &vecFormatStrings)
    {
         ...
         for (long unsigned int i = 0; i <= strOp.length(); i++)
         {
            ...
            {
               std::string strSubStr = strOp.substr(iLast, i - iLast);
    ...
    bool Apply(const SimplePath &path,
...
// --------------------------------------------------------------------------
void generate_path_segments_set_basic(
    ...
    for (size_t t = 0; t < vecIterCandidates.size()-1; t++)
    {
       ...
       for (long unsigned int i = 0; i < tscinfo.size(); i++)
       {
          ...
          for (int g = 1; ; g++)
          {
             ...
             int iRes = reinterpret_cast<int*>(kd_res_item_data(set))
                                     - reinterpret_cast<int*>(noptr);

             kd_res_free(set);

             // Great circle distance between points
             double dRDeg = GreatCircleDistance_Deg(vecNodes[t][i].lonRad,
                                                    vecNodes[t][i].latRad,
 ...
 */
void generate_path_segments_with_priority(
    const std::vector<TimeToCandidateInfoMapIterator> &vecIterCandidates,
    const std::vector<std::vector<Node>> &vecNodes,
    const std::vector<kdtree*> &vecKDTrees,
    ...
    std::vector<SimplePathSegmentSet> &vecPathSegmentsSet)
{
    ...
    for (auto & it : mapPriority)
    {
       ...
       for (int g = 1; ; g++)
       {
          ...
          kdres * set = kd_nearest_range3(vecKDTrees[t+g],
                                          vecNodes[t][i].x,
                                          vecNodes[t][i].y,
                                          vecNodes[t][i].z,
                                          dChordLength);
          ...
          do
          {
             ...
             double dRDeg = GreatCircleDistance_Deg(vecNodes[t][i].lonRad,
                                                    ...
                                                    vecNodes[t+g][iRes].lonRad,
                                                    vecNodes[t+g][iRes].latRad);
             mmapTargets.insert(std::pair<PriorityPair,
                                TimeCandidatePair>(PriorityPair(g,dRDeg),
    ...
    const std::string &prefix, options_description &global_opts)
{
    ...
    opts.add_options()
        ...
        TECA_POPTS_GET(std::string, prefix, track_threshold_cmd,
            "Threshold commands for path [col,op,value,count;...]")
        ...
        TECA_POPTS_GET(double, prefix, min_path_distance,
            "Minimum accumulated great-circle distance"
            " between nodes in a path (in degrees)")
    ...
    const std::string &prefix, variables_map &opts)
{
    ...
    TECA_POPTS_SET(opts, std::string, prefix, track_threshold_cmd)
    ...
    const teca_metadata &request)
{
    ...
    if (this->prioritize == "")
    {
       generate_path_segments_set_basic(vecIterCandidates,
                                        vecNodes,
                                        vecKDTrees,
    ...
    else
    {
       ...
       generate_path_segments_with_priority(vecIterCandidates,
                                            vecNodes,
                                            vecKDTrees,
                                            ixPriorityCol,
                                            this->internals->max_gap_steps,
    ...
    generate_paths_basic(vecIterCandidates,
                         vecPathSegmentsSet,
                         vecPaths);
