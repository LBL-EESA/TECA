...
// use one of the for key_map_t
// for key that are not ordinals 0 to N
template<typename key_t>
using mapped_key_t = std::map<key_t, unsigned long>;

// for contiguious keys 0 to N (faster)
using contiguous_key_t = std::vector<unsigned long>;

// for ward declare the queue
...
/**
an indirect priority queue that supports random access modification of priority
the queue works with user provided keys and lookup functor that converts
keys to priorities.
...
comp_t - callable that implements the predicate: bool(key_t, key_t),
         used to enforce heap order. (std::less<>)
...
create an instance of the priority_queue and push the key values. as
keys are pushed heap ording is imposed, this is why objects need to be
in place before pushing keys. when an objects prioriy has been changed
one must call modified passing the key of the object. the location
of each object is tracked and the queue will reprioritize itself after
modification.
...
template <typename key_t, typename lookup_t,
    typename comp_t, typename key_map_t>
void teca_priority_queue<key_t, lookup_t,
    comp_t, key_map_t>::to_stream(std::ostream &os)
{
