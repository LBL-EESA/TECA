...
int main(int argc, char **argv)
{
    ...
    if (argc < 6)
    {
        std::cerr << "error: wrong number of arguments" << std::endl
            << "usage: test_spatial_exzecutive [nx] [ny] [nz] [nt] "
               "[n_spatial_partitions] [temporal mode = \"size\"/\"number\"] "
    ...
    unsigned long number_of_spatial_partitions = atoi(argv[5]);
    ...
    if (argc == 8)
    {
        ...
        unsigned long mode_arg = atoi(argv[7]);
        if (mode == "size")
        {
    ...
    p_teca_space_time_executive exec = teca_space_time_executive::New();

    exec->set_number_of_spatial_partitions(number_of_spatial_partitions);
