...
const_p_teca_dataset create_test_table(long step, int tid)
{
    ...
    switch (tid)
    {
        case test_util::base_table:
            return t;
            break;

        case test_util::break_string_col:
            std::dynamic_pointer_cast<teca_variant_array_impl<std::string>>(
                t->get_column("name"))->set(2, "Dave");
            return t;
            break;

        case test_util::break_int_col:
            std::dynamic_pointer_cast<teca_variant_array_impl<std::string>>(
                t->get_column("age"))->set(2, 6);
            return t;
            break;

        case test_util::break_float_col:
            std::dynamic_pointer_cast<teca_variant_array_impl<std::string>>(
