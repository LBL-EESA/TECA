...
int main(int argc, char **argv)
{
    ...
    if (argc < 4)
    {
        cerr << endl << "Usage error:" << endl
            << "test_table_remove_rows_table [input table] [test baseline] [basin name]"
            << endl << endl;
        return -1;
    }
    std::string input_table = argv[1];
    std::string baseline_table = argv[2];
    std::string basin_name = argv[3];

    // create the pipeline
    p_teca_table_reader input_reader = teca_table_reader::New();
    input_reader->set_file_name(input_table);

    p_teca_table_region_mask reg_mask = teca_table_region_mask::New();
    reg_mask->set_input_connection(input_reader->get_output_port());
    reg_mask->load_cyclone_basin(basin_name);
    reg_mask->set_result_column("in_basin");

    p_teca_table_remove_rows eval_expr = teca_table_remove_rows::New();
    eval_expr->set_input_connection(reg_mask->get_output_port());
